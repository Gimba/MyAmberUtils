...
### return frame number of minimas
    min_indices = [h for h in hist2.argmin(axis=0)]
    minimas = [m for m in ]
...
###PLOTTING FUNCTION FOR FREE ENERGY FIGURE
    if plt_figs :
    	...
    	print extent
    	plt.imshow(hist2.transpose(), extent=extent, interpolation='gaussian', cmap='rainbow')
    	...
    	plt.xticks(size='18')
    	plt.yticks(size='18')
    	plt.xlabel(r'PCA1 ($\AA$)',fontsize=18)
    	plt.ylabel(r'PCA2 ($\AA$)',fontsize=18)
